...
export class QuarterMeetingsComponent implements OnInit {
  ...
  public years = ['2010', '2011', '2012', '2013', '2014', '2015', '2016', '2017', '2018'];
  constructor(private http: Http) {
  }
  displayHeatMap: (year) => void
  = function(year): void {
    d3.select("svg").remove();
    this.year1 = parseInt(year);
    ...
    this.http.get('/generateheatmap', { headers: contentHeaders })
      ...
      response => {
        ...
        rect.on("click", function(d) {
          self.http.get('/showmoredetails/' + d, { headers: contentHeaders })
            .subscribe(
            response => {
              self.d = d;
              self.details = response.json();
              self.formatteddate = self.details[0].meetingDate;
        ...
        rect.filter(function(d) { return d in data; })
          ...
          .text(function(d) {
            var date = d.split("-");
            this.datenew = date[2] + "-" + date[1] + "-" + date[0];
            return this.datenew + ":" + data[d];
          });
